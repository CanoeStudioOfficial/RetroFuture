...
public class OpenSimplexNoise {
    ...
    static {
        ...
        for(int idx = 0; idx < grad2.length; ++idx) {
            grad2[idx] = (float)((double)grad2[idx] / 0.05481866495625118);
        }

        int index = 0;
        for(int j = 0; index < GRADIENTS_2D.length; ) {
            if (j == grad2.length) {
                j = 0;
            }
            GRADIENTS_2D[index] = grad2[j];
            ++index;
            ++j;
        }
        ...
        for(int idx = 0; idx < grad3.length; ++idx) {
            grad3[idx] = (float)((double)grad3[idx] / 0.2781926117527186);
        }

        int grad3Index = 0;
        for(int j = 0; grad3Index < GRADIENTS_3D.length; ) {
            if (j == grad3.length) {
                j = 0;
            }
            GRADIENTS_3D[grad3Index] = grad3[j];
            ++grad3Index;
            ++j;
        }
        ...
        for(int idx = 0; idx < grad4.length; ++idx) {
            grad4[idx] = (float)((double)grad4[idx] / 0.11127401889945551);
        }

        int grad4Index = 0;
        for(int j = 0; grad4Index < GRADIENTS_4D.length; ) {
            if (j == grad4.length) {
                j = 0;
            }
            GRADIENTS_4D[grad4Index] = grad4[j];
            ++grad4Index;
            ++j;
        }
        ...
        // 生成顶点映射
        for(int vertexIdx = 0; vertexIdx < 256; ++vertexIdx) {
            int cx = (vertexIdx >> 0 & 3) - 1;
            int cy = (vertexIdx >> 2 & 3) - 1;
            int cz = (vertexIdx >> 4 & 3) - 1;
            int cw = (vertexIdx >> 6 & 3) - 1;
            latticeVerticesByCode[vertexIdx] = new LatticeVertex4D(cx, cy, cz, cw);
        }
        ...
        for(int tableIdx = 0; tableIdx < 256; ++tableIdx) {
            nLatticeVerticesTotal += lookup4DVertexCodes[tableIdx].length;
        }

        // 初始化查找表
        LOOKUP_4D_A = new int[256];
        LOOKUP_4D_B = new LatticeVertex4D[nLatticeVerticesTotal];
        int lookupPos = 0; // 当前位置指针

        for(int tableIdx = 0; tableIdx < 256; ++tableIdx) {
            int startPos = lookupPos;
            int length = lookup4DVertexCodes[tableIdx].length;

            // 记录起始位置和长度（高16位存储结束索引）
            LOOKUP_4D_A[tableIdx] = startPos | ((startPos + length) << 16);

            // 填充顶点数据
            for(int k = 0; k < length; ++k) {
                LOOKUP_4D_B[lookupPos++] =
                        latticeVerticesByCode[lookup4DVertexCodes[tableIdx][k]];
            }
        }
    }
