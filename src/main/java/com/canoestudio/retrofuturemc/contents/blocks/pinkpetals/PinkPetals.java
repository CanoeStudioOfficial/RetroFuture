...
public class PinkPetals extends BlockCarpet {
    public PinkPetals(String name) {
        ...
        /*设置声音*/setSoundType(new SoundPinkPetals());
    ...
    //元数据列表
    /*元数据列表1: 获取元数据-通过状态*/private final Map<List<Integer>, Integer> getMetaFromState_Map = new HashMap<>();
    /*元数据列表2: 获取状态-通过元数据*/private final Map<Integer, List<Integer>> getStateFromMeta_Map = new HashMap<>();
    //初始化元数据列表
    ...
    public void neighborChanged(IBlockState state, World world, BlockPos pos, Block block, BlockPos fromPos) {
        if (PinkpetalsItem.downBlockIsGrass(world, pos)) {
            return;
        }
